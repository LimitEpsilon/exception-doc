...
\begin{prooftree}
  ...
  \infer[left label={[DECON]}]1[in case]{
    {\begin{array}{r l}
       ...
       & V(e,\sigma) \supseteq
       \mathsf{con}^{-1}_{(2)}(V(e',\sigma)\cap \mathsf{con}(\kappa, \_))\:\wedge \\
       & V(e,\sigma) \supseteq
...
\begin{prooftree}
  ...
  \infer[left label={[\underline{DECON}]}]1[in handle]{
       ...
       V(e,\sigma) \supseteq
       \mathsf{exn}^{-1}_{(2)}(\underbrace{P(e',\sigma)\cap \mathsf{con}(\kappa, \_)}_{\text{filter}}) \wedge C'
     }
\end{prooftree}
\vspace{0.2cm}
...
\begin{prooftree}
  ...
  \infer[left label={[HANDLE]}]2{
    {\begin{array}{r l}
       ...
       &P(e,\sigma) \supseteq (P(e',\sigma\vbar_{e'})\cap\bigcap_{i=1}^{n}\loverbar{\mathsf{exn}(p_i, \_)})\:\wedge\\
